...
use reqwest::{Body, Client, Method, header::CONTENT_TYPE};
...
use crate::{
    ...
    provider_mode::{PROVIDER_API_KEY_HEADER, ProviderMode, SupportedProviderMode, proxy::insert_proxied_headers_into},
    ...
    pub fn new(name: String, config: ApiProviderConfig) -> crate::Result<Self> {
        ...
        if config.forward_token {
            supported_modes.push(SupportedProviderMode::RouterWithClientKey(
                http::HeaderName::from_static(PROVIDER_API_KEY_HEADER),
            ));
        }
...
impl Provider for AnthropicProvider {
    async fn chat_completion(
        ...
        let url = format!("{}/messages?beta=true", self.base_url);
        ...
        let body = Bytes::from(sonic_rs::to_vec(&anthropic_request).map_err(|e| {
            log::error!("Failed to serialize Anthropic request: {e}");
            LlmError::InternalError(None)
        })?);

        let mut body: serde_json::Value = serde_json::from_slice(&context.body).unwrap();
        let obj = body.as_object_mut().unwrap();
        obj.insert("model".into(), "claude-sonnet-4-5-20250929".into());
        ...
        log::debug!("Anthropic Sent HTTP Request:");
        log::debug!("  Method: {}", req.method());
        log::debug!("  URL: {}", req.url());
        log::debug!("  Headers: {:?}", req.headers());
        // log::debug!(
        //     "  Body: {}",
        //     serde_json::to_string_pretty(&serde_json::from_slice::<serde_json::Value>(&body).unwrap()).unwrap()
        // );
